...
export default function AdminPage() {
  ...
  const load = async () => setContent(await (await fetch("/api/admin/content")).json());
  useEffect(() => { load(); }, []);
  ...
  return <div className="p-6 max-w-5xl mx-auto text-white">
    ...
    <div className="grid md:grid-cols-3 gap-3 mb-6">
      ...
      <div className="flex gap-2"><button className="bg-accent px-3" onClick={()=>post('/api/admin/bootstrap',{username:form.username,password:form.password})}>Create Admin</button><button className="bg-accent px-3" onClick={()=>post('/api/admin/login',{username:form.username,password:form.password})}>Login</button></div>
    </div>
    ...
    <textarea className="w-full h-[400px] text-black p-2" value={JSON.stringify(content,null,2)} onChange={(e)=>setContent(JSON.parse(e.target.value||"{}"))}/>
    <button className="bg-accent px-4 py-2 mt-3" onClick={async()=>{const r=await fetch('/api/admin/content',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(content)});setMsg(r.ok?'Saved':'Save error')}}>Save Content</button>
